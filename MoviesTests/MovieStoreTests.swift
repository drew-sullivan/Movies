...
class MovieStoreTests: XCTestCase {
    ...
    func test_fetchingMovieMetadata_createsMovieObjects() {
        ...
        sut.fetchMovieMetadata(by: .nowPlaying) { (movieResult) in
            switch movieResult {
    ...
    func test_fetchingMovieMetadata_whenJSONIsInvalid_returnsError() {
        ...
        sut.fetchMovieMetadata(by: .nowPlaying) { (movieResult) in
            switch movieResult {
                case .success(let movies):
                    XCTAssertNil(movies, "This should have thrown an error")
    ...
    func test_fetchingMovieMetadata_whenDataIsNil_returnsError() {
    ...
    func test_fetchingMovieMetadata_whenResponseHasAnError_returnsError() {
        let error = NSError(domain: "FakeError", code: 1234, userInfo: nil)
